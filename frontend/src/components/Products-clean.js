  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editing) {
        ...
        setMessage('PKR œ… Product updated successfully!');
      } else {
        await axios.post('http://localhost:5000/products', form, { headers: { Authorization: `Bearer ${token}` } });
        setMessage('PKR œ… Product added successfully!');
      }
  ...
  async function handleDelete(id) {
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        ...
        setMessage('PKR œ… Product deleted successfully!');
        fetchProducts();
        setTimeout(() => setMessage(null), 3000);
      } catch (err) {
            ...
            <div className="grid grid-cols-4 gap-6">
              ...
              {view === 'products' ? (
                ...
                <>
                  ...
                  <div className="professional-search-group">
                    <label className="professional-label">
                      <span className="label-icon">ï¿½ðŸª</span>
                      Sale Type
                    </label>
                    <div className="input-wrapper">
                      <select
                      ...
                      <div className="select-arrow">PKR –¼</div>
                    </div>
                  </div>
                  
                  <div className="professional-search-group">
                    <label className="professional-label">
                      <span className="label-icon">ï¿½ðŸ“…</span>
                      Start Date
                    </label>
                    <div className="input-wrapper">
                      <input
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="card-title">
                      {editing ? 'PKR œï¸ Edit Product' : 'ðŸ“¦ Product Management'}
                    </h2>
                  ...
                  {!editing && (
                    ...
                    >
                      {showAddForm ? (
                        <>
                          PKR œ–ï¸ <span>Cancel</span>
                        </>
                      ) : (
                        <>
                          PKR ž• <span>Add New Product</span>
                        </>
                      )}
                    </button>
                  )}
              ...
              {(showAddForm || editing) && (
                ...
                <form onSubmit={handleSubmit} className="professional-form">
                  <div className="professional-form-grid">
                    ...
                    <div className="professional-input-group">
                      <label className="professional-label">
                        <span className="label-icon">ðŸ·ï¸</span>
                        Brand
                      </label>
                      <div className="input-wrapper">
                        <input 
                    ...
                    <div className="professional-input-group supplier-input-group">
                      ...
                      <div className="input-wrapper">
                        <div className="supplier-search-wrapper">
                          ...
                          >
                            <span className={`dropdown-arrow ${supplierDropdownOpen ? 'open' : ''}`}>PKR –¼</span>
                          </button>
                          <div className="input-focus-border"></div>
                        </div>
                  ...
                  <div className="professional-form-actions">
                    <button type="submit" className="btn-professional btn-professional-primary">
                      <span className="btn-icon">{editing ? 'ðŸ’¾' : 'PKR ž•'}</span>
                    ...
                    >
                      <span className="btn-icon">PKR œ–ï¸</span>
              ...
              <div className="modal-overlay">
                <div className="modal-content">
                  <div className="modal-header">
                    <h3 className="modal-title">ðŸ“Š Barcode for {barcodeProduct.name}</h3>
                  </div>
                ...
                <div className="grid grid-cols-4 gap-4 mb-6">
                  ...
                  <div className="bg-gradient-to-r from-orange-50 to-orange-100 p-4 rounded-lg border border-orange-200">
                    ...
                    <div className="text-2xl font-bold text-orange-900">
                      PKR ‚¹{sales.reduce((sum, sale) => sum + parseFloat(sale.discount || 0), 0).toFixed(2)}
                    </div>
                  </div>
                  <div className="bg-gradient-to-r from-green-50 to-green-100 p-4 rounded-lg border border-green-200">
                    <div className="text-sm font-medium text-green-600">Net Revenue</div>
                    <div className="text-2xl font-bold text-green-900">
                      PKR ‚¹{(sales.reduce((sum, sale) => sum + parseFloat(sale.total_amount || 0), 0) - sales.reduce((sum, sale) => sum + parseFloat(sale.discount || 0), 0)).toFixed(2)}
                    </div>
                  </div>
                  ...
                  <table className="table">
                    <thead>
                      <tr>
                        ...
                        <th>ï¿½ Customer</th>
                        <th>ï¿½ Brand Name</th>
                        <th>ï¿½ðŸª Sale Type</th>
                        <th>ï¿½ðŸ“Š Status</th>
                        <th>ðŸ’° Total Amount</th>
                        <th>ðŸŽ¯ Discount</th>
                        <th>ðŸ“¦ Items Sold</th>
                      </tr>
                    </thead>
                          ...
                          <tr key={sale.id}>
                            ...
                            <td>
                              <div className={`px-2 py-1 rounded text-sm font-medium ${
                                ...
                                {sale.sale_type === 'wholesale' || sale.customer_id ? 'ðŸ¢ Wholesale' : 'ðŸ›ï¸ Retail'}
                              </div>
                            </td>
                            <td>
                            ...
                            <td>
                              <div className="font-semibold text-warning">
                                {sale.discount && parseFloat(sale.discount) > 0 ? (
                                  <>
                                    PKR ‚¹{parseFloat(sale.discount).toFixed(2)}
                                    <div className="text-xs text-gray-500">
                                ...
                                {sale.items && sale.items.map((item, index) => (
                                  <div key={item.id || index} className="mb-1 p-2 bg-gray-50 rounded border-l-4 border-blue-500">
                                    ...
                                    <div className="text-xs text-gray-600 mt-1">
                                      PKR ‚¹{parseFloat(item.price).toFixed(2)} each = PKR ‚¹{(parseFloat(item.price) * parseInt(item.quantity)).toFixed(2)}
                                    </div>
                                  </div>
                                ))}
                              </div>
