    ...
    try {
        ...
        if (revenueDiff < 0.01 && discountDiff < 0.01) {
            ...
            sales.slice(0, 3).forEach((sale, i) => {
                console.log(`${i + 1}. ID ${sale.id}: Amount ${sale.total_amount}, Discount ${sale.discount}`);
            });

            console.log('\nSample Sold Products (first 3):');
            soldProductsData.soldProducts.slice(0, 3).forEach((product, i) => {
                console.log(`${i + 1}. ${product.product_name}: Net Revenue ${product.total_net_revenue}`);
            });
        }
